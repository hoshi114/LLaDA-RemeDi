...
try:
    ...
    load_dataset = None  # optional; only needed when building HF datasets

from transformers import AutoTokenizer

from remedi.modeling_wrappers import UPSHead, get_last_hidden_states
...
def collate_batch(batch: List["SFTExample"], pad_id: int) -> Tuple[torch.Tensor, torch.Tensor]:
    """Collate to rectangular tensors for SFT examples.
...
def compute_losses(model,
    ...
    If freeze_backbone=True, the backbone forward is run under no_grad (UPS-only SFT).
    Otherwise, gradients flow through the backbone for the diffusion loss (LoRA/TPS training).
    In both cases, the UPS BCE backpropagates only to the UPS head (hidden is detached).
    """
    # Backbone forward
    if freeze_backbone:
        with torch.no_grad():
    ...
    # UPS loss: build labels for all positions; stop-grad on y
    with torch.no_grad():
    ...
    # Detach hidden so BCE won't affect backbone even when not frozen
    h_in = hidden.detach()
    head_dtype = next(ups_head.parameters()).dtype
    bce = F.binary_cross_entropy_with_logits(
        ups_head(h_in.to(head_dtype)).float(),
        y.float(),
        reduction='mean'
    )
